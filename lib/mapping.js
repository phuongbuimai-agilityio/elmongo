...
exports.generateMapping = function (schema) {
    ...
    var pathArray = path.split('.')
		...
		pathArray.forEach(function (pathEntry, i) {
          ...
          var instanceName = pathInfo.instance.toLowerCase()
            ...
            var type = pathInfo.caster.instance.toLowerCase()
				...
				// set index type
				if (pathInfo.options && pathInfo.options.index) {
          if (pathInfo.options.index == 'multi_field' && pathInfo.options.indexType === 'completion') {
            currentLocation[pathEntry].type = 'completion';
            currentLocation[pathEntry].fields = pathInfo.options.fields;
          } else if (pathInfo.options.index == 'multi_field' && pathInfo.options.fields && pathInfo.options.indexType !== 'geo_point') {
						// multi field index
						currentLocation[pathEntry].type = 'text';
            currentLocation[pathEntry].fields = pathInfo.options.fields;
					} else if (pathInfo.options.index == 'not_analyzed') {
						// not analyzed index
            currentLocation[pathEntry].index = 'keyword';
        ...
        if (pathInfo.options && pathInfo.options.index === 'keyword') {
          currentLocation[pathEntry].type = 'keyword';
        }

        if (pathInfo.options && pathInfo.options.index === 'string') {
          currentLocation[pathEntry].type = 'text';
        }
  ...
  function eachRecursive(obj) {
    for (var k in obj) {
      if (!obj.fields && obj.index !== 'keyword' && obj.index !== 'multi_field' && obj[k] === 'string' || obj[k] === 'text') {
        obj['fielddata'] = true;
      }
