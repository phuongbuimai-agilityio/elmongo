...
exports.serializeModel = function (model, options) {
    ...
    var toObjectOptions = { depopulate: true };

    if (options.transform) {
        toObjectOptions.transform = options.transform;
    }

    // strip mongoose-added functions, and depopulate any populated model references
    var deflated = model.toObject(toObjectOptions);
...
exports.mergeSearchBody = function (searchOpts) {
    ...
    var defaultQuery = searchOpts.query;

    if (typeof searchOpts.query === 'string') {
        defaultQuery = {
            bool: {
                should: [
    ...
    // Thuong Dinh: modified to help extend search command
    var body = {
        query: defaultQuery,
        sort: searchOpts.sort
    }

    // Huy Tran: add search fields
    if (searchOpts.fields && typeof searchOpts.fields === 'object' && searchOpts.fields.indexOf('_all') === -1)
        body['fields'] = searchOpts['fields']

    if (searchOpts._source && typeof searchOpts._source === 'object') {
        delete body['fields'];
        body._source = searchOpts._source;
        console.log('option of source is here');
    }

    // console.log('Elasticsearch Body', JSON.stringify(body, null, 4));
    // console.log(searchOpts._source);
    ...
    exports.backOffRequest(reqOpts, function (err, res, body) {
        ...
        if (body.hits.hits && body.hits.hits.length) {
            ...
            searchResults.hits.map(function(hit) {
                var source = hit && hit._source;

                if (source) {
                    hit._source._id = source._id || hit._id;
                }
...
exports.makeDomainUri = function (options) {
    // modified to help check authentication
    var host = options && options.host ? options.host : '';
    var isCheckAuth = host.includes('@');
    // split host and auth from host option
    var hostAuth = isCheckAuth ? host.split('@')[0] : '';
    var hostDomain = isCheckAuth ? host.split('@')[1] : host;
